...
class TelemetryEngine {
  ...
  async getVirtualizationOverview() {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 3) {
            const id = parts[0];
            const name = parts[1];
            const status = parts.slice(2).join(' ').toLowerCase();
            vms.push({
              id,
              name,
              status: status.includes('running') ? 'running' : status.includes('paused') ? 'paused' : 'shut off',
              vcpus: 2,
              memoryMb: 2048,
    ...
    for (const [ifaceName, ifaceDetails] of Object.entries(ifaces)) {
      if (ifaceName.startsWith('docker') || ifaceName.startsWith('virbr') || ifaceName.startsWith('br-') || ifaceName.startsWith('veth') || ifaceName.startsWith('tun') || ifaceName.startsWith('tap')) {
        ...
        bridges.push({
          ...
          type: ifaceName.startsWith('docker') ? 'docker' : ifaceName.startsWith('virbr') ? 'libvirt' : ifaceName.startsWith('veth') ? 'veth' : 'bridge',
          state: 'active'
        });
      }
    }
